...
const dataObserver = {
    next: (data) => {
        ...
        const bricks = data.split('\n').filter((line) => !!line).reduce(scanBricks, [])
                ...
                .reduce(dropBricks, []);
        const disintegrateBricks = bricks.reduce((disintegrateBricks, brick) => {
            if (brick.supports.every(supportedBrick => supportedBrick.supportedBy.length > 1)) disintegrateBricks.push(brick);
            return disintegrateBricks;
        }, []);
        console.log(disintegrateBricks.length);
        console.timeEnd(id);
    }
};
