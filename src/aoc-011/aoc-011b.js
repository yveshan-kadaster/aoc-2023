...
const expansionFactor= 10; // 1000000
...
function expandMap(map) {
    ...
    return {
        width: map.width + expansionX[expansionX.length - 1] * (expansionFactor - 1),
        height: map.height + expansionY[expansionY.length - 1] * (expansionFactor - 1),
...
function findExpansion(size, included) {
    ...
    return empty.reduce((p, c, i) => {
        return Array(c).fill(i * (expansionFactor - 1)).map((v, i) => i < p.length ? p[i] : v);
    }, []);
}
