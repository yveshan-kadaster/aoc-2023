...
function createDigPlan(digPlan, line) {
    ...
    const num2heading = { '0': 'R', '1': 'D', '2': 'L', '3': 'U' }
    digPlan.push({ heading: num2heading[match[3].charAt(5)], size: parseInt(match[3].substring(0, 5), 16), color: match[3] });
    return digPlan;
}
...
fs.readFile('952408144115b.txt', 'utf8', (err, data) => {
    if (err) {
        console.error(err);
        return;
    }
